...
# CryptoCompare API Key (Replace with your key)
API_KEY = "YOUR_CRYPTOCOMPARE_API_KEY"

# Cache data fetching to avoid redundant API calls
@st.cache_data(ttl=60)  # Cache data for 60 seconds
def fetch_data(symbol, interval, limit=100):
    url = f"https://min-api.cryptocompare.com/data/v2/histominute" if interval == "1m" else \
          f"https://min-api.cryptocompare.com/data/v2/histohour" if interval == "1h" else \
    ...
    try:
        ...
        return df[["datetime", "open", "high", "low", "close", "volumeto"]].rename(columns={"volumeto": "volume"})
